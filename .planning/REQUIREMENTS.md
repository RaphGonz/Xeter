...
- [ ] **INFRA-02**: TraceAnalyzer score data is persisted — `flush_scores()` called and results written to `span_scores` after each trace flush; calibration dataset includes trace-level metrics
...
- [ ] **INFRA-05**: `SpanData` has `expected_output_schema: Optional[dict]` field; corresponding ClickHouse column added (additive, nullable); `span_fetcher.py` maps it
- [ ] **INFRA-06**: `SpanData` has `parent_span_id: Optional[str]` field; confirmed present or added to ClickHouse `spans` table; `span_fetcher.py` maps it
