...
- **DIAG-05**: DiagnosisRepository DAL persists LLM diagnoses to PostgreSQL with tenant isolation (create, get_latest_for_span)
- **DIAG-06**: POST /diagnose endpoint accepts span_id, assembles context, calls LLM, stores and returns structured diagnosis
- **DIAG-07**: Diagnosticer service has unit test coverage for success (200), auth failure (401), span not found (404), LLM error (502), and parse error (422) paths
