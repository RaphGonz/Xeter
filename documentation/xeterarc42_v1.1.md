...
arc42 Template | Version 1.1 — Post-implementation
Audience: Developers
...
The v1.1 milestone focused entirely on analyser accuracy: replacing four conceptually-wrong check methods with research-backed implementations, each calibrated against a labelled fixture and required to pass P/R benchmarks before the next method was touched.
...
| SDK secondary language: TypeScript | Deferred to v1.2 |
...
| `_check_wrong_tool_choice` | `wrong_tool_choice` | spaCy lemma containment guard → embedding ranking; flags if rank > 1 and top tool is coherent |
...
| `_check_no_tool` | `no_tool` | Embed prompt vs "call a function tool" reference; flag if similarity > threshold |
...
- POST /login / POST /register / POST /diagnose
...
- Span list, FilterBar, span detail Sheet with FlagSection and PayloadTabs

**Storage:**
- **ClickHouse** — spans (immutable, append-only)
- **PostgreSQL** — flags, span_scores, tenants, users, api_keys
...
Worker (BRPOP loop)
    ...
    |     if rank > 1 AND top score ≥ tool_coherence_threshold → flag wrong_tool_choice
    |
    ...
    |     if score > no_tool threshold → flag no_tool
    |
...
```
View
    |-- GET /spans?flag_type=wrong_tool_choice&agent_name=...&from_time=...
...
Three stores. Spans are immutable once written. Flags and scores are separate append-only tables in PostgreSQL, referenced by `span_id`.
...
`flag_type` values in v1.1: `tool_not_available`, `wrong_tool_choice`, `unnecessary_tool_call`, `wrong_tool_args`, `no_tool`, `parsing_error`, `response_anomaly`.
...
| span_id | String | |
| comparison | VARCHAR | metric name e.g. `embedding_rank`, `arg_grounding`, `social_centroid_score` |
| score | Float | |
| created_at | Timestamp | |
...
| `wrong_tool_choice` | spaCy containment guard + embedding rank | Rank-based | P=1.0, R=0.5 |
...
| `no_tool` | Prompt vs reference embedding | Threshold (0.15) | P=1.0, R=0.333 |
...
- **Diagnosticer unreachable:** Presenter catches `httpx.HTTPError`, returns 502.
- **S3 payload fetch times out (Presenter):** `asyncio.wait_for` → 504.
...
| AD16 | SDK secondary language: TypeScript | Decided | Deferred to v1.2 |
...
|----|-----------------|-------------------|
| QS01 | Correctness | Span where prompt is "schedule a meeting" and `tool_name` is "get_weather" | `wrong_tool_choice` flag; rank and top_candidate in detail |
...
| QS04 | Availability | LLM provider unreachable | Diagnosticer returns 502; existing data unaffected |
...
| QS07 | Correctness | Span where called tool is not the top-ranked match in available_tools (and lemma containment fails) | `wrong_tool_choice` flag with ranked candidate list in detail |
...
| R06 | S3 payload retrieval strategy | Medium | Resolved | Lazy loading; `asyncio.wait_for` with timeout |
| R07 | diagnostics.result JSON structure | Low | Open | Diagnosticer inactive in v1; structure defined for v2 |
| R08 | Dual SDK maintenance (Python + TypeScript) | Medium | Deferred | TypeScript SDK deferred to v1.2 |
...
| R15 | `wrong_tool_choice` recall is low (R=0.5) | Medium | Accepted | Conservative by design — precision target ≥ 80% set; high recall deferred to v2 when LLM-based diagnosis can catch missed cases |
...
| Diagnostic | A row in the PostgreSQL diagnostics table written by the Diagnosticer (inactive v1) |
...
| Diagnosticer | Separate service for LLM-based trace analysis (scaffolded in v1, active in v2) |
...
| wrong_tool_choice | Flag type: a better tool existed among available tools (rank > 1, coherent top match) |
...
| no_tool | Flag type: prompt implies tool use but no tool was called |
...
Document status: v1.1 — post-implementation. Analyser Accuracy milestone complete. Open items: diagnostics.result structure (v2), TypeScript SDK (v1.2), cloud deployment, wrong_tool_choice recall improvement (v2).
